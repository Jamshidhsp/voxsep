...
def rot_rand(x):
    # x = x.detach().clone()
    # device = 'curot_randda' if torch.cuda.is_available() else 'cpu'
    # print(x.shape)
...
def get_non_overlapping_crops(image_tensor, crop_size):
    b, c, _, _, _ = image_tensor.shape
    
    crops = image_tensor.unfold(2, crop_size[0], crop_size[0]) \
                        .unfold(3, crop_size[1], crop_size[1]) \
                        .unfold(4, crop_size[2], crop_size[2])
    crops = crops.reshape(b, c, -1, *crop_size)
    crops = crops.mean(dim=(-3, -2, -1))
    
    return crops.transpose(1, 2).reshape(-1, crops.size(1))

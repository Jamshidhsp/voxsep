...
class Vox2Vec(pl.LightningModule):
    def __init__(
        ...
        self.temp = temp
        self.lr = lr
        self.queue = Queue(max_size=60000, embedding_size=proj_dim)
    ...
    def _vox_to_vec_key(self, patches: torch.Tensor, voxels: Iterable[torch.Tensor]) -> torch.Tensor:
        feature_pyramid = self.encoder_key(patches)[:]
        return torch.cat([select_from_pyramid([x[j] for x in feature_pyramid], v) for j, v in enumerate(voxels)])


    def update_momentum_encoder(self):
        for param_q, param_k in zip(self.backbone.parameters(), self.encoder_key.parameters()):
    ...
    def training_step(self, batch, batch_idx):
        ...
        embeds_1_positive = self.proj_head(self._vox_to_vec_key(patches_1, [voxel_list_positive.view(-1, 3)]))
        ...
        embeds_2 = self.proj_head(self._vox_to_vec_key(patches_2, [voxel_list_negative.view(-1, 3)]))
        # self.queue.update(embeds_2)
        ...
        for i in range(embeds_1.size(0)):
            ...
            emb1_p = embeds_1_positive[i] # shape is (num_posive, 128)
            emb2 = embeds_2[i]
            

            # emb1 = torch.logsumexp(embeds_1[i],dim=1).unsqueeze(1) # shape is (num_posive, 128)
            # emb2 = torch.logsumexp(embeds_2[i],dim=1).unsqueeze(1)


            self.queue.update(emb2)
            queue = self.queue.get().detach().to(emb1.device)
            # print(queue[0][0])
            l_pos_1 =  torch.einsum('pe, qe->pq', [emb1, emb1_p])
            # l_pos_1 =  torch.einsum('pe, qe->p', [emb1, emb1]).unsqueeze(1)
            # l_pos_1.fill_diagonal_(float('0'))
            print('max(l_pos_1)', torch.max(l_pos_1))
            print('mean(l_pos_1)', torch.mean(l_pos_1))
            l_pos_2 = torch.einsum('pe, qe->pq', [emb2, emb2])
            # l_pos_2 = torch.einsum('pe, qe->p', [emb2, emb2]).unsqueeze(1)
            # l_pos_2.fill_diagonal_(float('0'))
            print('max(l_pos_2)', torch.max(l_pos_2))
            print('mean(l_pos_2)', torch.mean(l_pos_2))
            # l_pos_1 = torch.exp(l_pos_1)
            # l_pos_2 = torch.exp(l_pos_2)
            l_neg_1 =  torch.einsum('pe,ke->pk', [emb1, queue])
            # l_neg_1 =  torch.einsum('pe,ke->p', [emb1, emb2])

            print('max(l_neg_1)', torch.max(l_neg_1))
            # l_neg_1 = torch.exp(l_neg_1)
            
            # l_neg_2 =  torch.einsum('pe,ke->pk', [emb2, emb2])
            logits_pos_1 = l_pos_1/self.temp
            print('max(logits_pos_1)', torch.max(logits_pos_1))
            logits_pos_2 = l_pos_2/self.temp
            print('max(logits_pos_2)', torch.max(logits_pos_2))
            logits_neg_1 = l_neg_1/self.temp
            print('max(logits_neg_1)', torch.max(logits_neg_1))
            print('mean(logits_neg_1)', torch.mean(logits_neg_1))
            # logits_neg_2 = l_neg_2/self.temp
            
            # logits_pos_1.fill_diagonal_(float('0'))
            # logits_pos_2.fill_diagonal_(float('0'))
            self.queue.update(emb1)
            
            labels_pos = 10*torch.ones((logits_pos_1.shape ), dtype=torch.float).to(l_pos_1.device)
            labels_pos_2 = 10*torch.ones((logits_pos_2.shape ), dtype=torch.float).to(l_pos_1.device)
            loss_ce_p_1 = F.mse_loss(logits_pos_1, labels_pos)/emb1.shape[0]
            loss_ce_p_2 = F.mse_loss(logits_pos_2, labels_pos_2)/emb1.shape[0]
            labels_neg = -10*torch.ones((logits_neg_1.shape ), dtype=torch.float).to(l_neg_1.device)
            loss_ce_n_1 = F.mse_loss(logits_neg_1, labels_neg)/emb1.shape[0]
            # loss_ce_n_2 = F.mse_loss(logits_neg_2, labels_neg)/emb2.shape[0]
            
            # self.queue.update(emb1)

            loss = (loss_ce_p_1+loss_ce_n_1)/2

            # loss_1 = torch.mean(torch.logsumexp(logits_pos_1, dim=1))
            # loss_2 = torch.mean(torch.logsumexp(logits_pos_2, dim=1))
            
            # a = torch.cat([logits_pos_1, logits_pos_2, logits_neg_1], dim=1)
            # loss_1 = torch.mean(-torch.logsumexp(l_pos_1, dim=1) +torch.logsumexp(a, dim=1))
            # loss_2 = torch.mean(-torch.logsumexp(l_pos_2, dim=1) +torch.logsumexp(a, dim=1))
            # loss = (loss_1+loss_2)/2
            # loss_2 = torch.mean(torch.logsumexp(torch.cat([logits_neg_1, logits_neg_2], dim=1), dim=1))
            # loss_2 = torch.mean(-logits_neg.diag()+torch.logsumexp(torch.cat([logits_neg.T, logits_pos_2], dim=1), dim=1))
            # loss = (loss_1 ) / 2
            # loss_1 = torch.mean(-logits_neg_1.diag() + torch.logsumexp(torch.cat([logits_pos_1, log], dim=1), dim=1))
            ...
            # print('loss', loss)
            running_loss =+ loss
            # self.log(f'pretrain/l_pos_1', loss_ce_p_1, on_epoch=True)
            # self.log(f'pretrain/l_pos_2', loss_ce_p_2, on_epoch=True)
            # self.log(f'pretrain/l_neg_1', loss_ce_n_1, on_epoch=True)
            # self.log(f'pretrain/l_neg_2', loss_ce_n_2, on_epoch=True)
        ...
        # for param_q, param_k in zip(self.backbone.parameters(), self.encoder_key.parameters()):
        #     print(param_q.data == param_k.data)
        # print('####################################################end')

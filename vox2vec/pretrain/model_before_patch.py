...
class Vox2Vec(pl.LightningModule):
    def __init__(
        ...
        self.lr = lr
        self.queue = Queue(max_size=10000, embedding_size=embed_dim)
    ...
    #     A_square = torch.sum(A ** 2, dim=1, keepdim=True)  # Shape: (N, 1)
    #     B_square = torch.sum(B ** 2, dim=1, keepdim=True).T  # Shape: (1, M)
        
    #     distances = A_square + B_square - 2 * torch.matmul(A, B.T)
    ...
    # def spatial_aware_loss(embeds_anchor, embeds_positive, embeds_key, margin=1.0):
    #     # Calculate pairwise distances
    #     anchor_positive_dist = pairwise_distances(embeds_anchor, embeds_positive)  # Shape: (N, N)
    #     anchor_key_dist = pairwise_distances(embeds_anchor, embeds_key)  # Shape: (N, M)

    #     # Positive loss: Diagonal elements of anchor-positive distance matrix
    #     positive_loss = torch.mean(anchor_positive_dist.diag())

    #     # Negative loss: Use broadcasting for identity matrix to match the shape of anchor_key_dist
    #     identity_matrix = torch.eye(anchor_positive_dist.size(0)).to(embeds_anchor.device)
    #     identity_matrix = identity_matrix.repeat(1, anchor_key_dist.size(1))  # Shape: (N, M)

    #     negative_loss = torch.mean(F.relu(margin - anchor_key_dist + identity_matrix * margin))

    #     # Final spatial-aware loss
    #     loss = positive_loss + negative_loss

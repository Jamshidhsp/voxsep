...
class Vox2Vec(pl.LightningModule):
    ...
    #     feature_pyramid = self.encoder_key(patches)[:-3]
    #     return torch.cat([select_from_pyramid([x[j] for x in feature_pyramid], v) for j, v in enumerate(voxels)])


    def update_momentum_encoder(self):
        for param_q, param_k in zip(self.encoder_key.parameters(), self.backbone.parameters()):
            param_k.data = param_k.data * 0.998 + param_q.data * (1. - 0.998)
    ...
    def training_step(self, batch, batch_idx):
        ...
        # assert self.backbone.training==False
        assert self.proj_head.training
        # assert self.encoder_key.training
        ...
        embeds_1_positive = self.proj_head(self._vox_to_vec(patches_1, [voxel_list_positive.view(-1, 3)]))
        ...
        # negative logits: NxK
        l_neg = torch.einsum('nc,kc->nk', [embeds_1, self.queue.get().detach().to(embeds_1.device)])
        N = l_pos.size(0)
        logits = torch.cat((l_pos, l_neg), dim=1)
        logits /= self.temp
        labels = torch.zeros((N, ), dtype=torch.long).to(l_pos.device)
        # labels = torch.ones(logits.shape[])

        loss = F.cross_entropy(logits, labels)
        
        
        
        # pos_labels = torch.zeros(embeds_1.shape[0]).to(embeds_1.device)
        # neg_labels = torch.ones(embeds_2.shape[0]).to(embeds_1.device)
        # # labels = torch.cat((pos_labels, neg_labels),axis=1)
        # pos_loss = criterion(embeds_1_positive, embeds_2, pos_labels)
        # neg_loss = criterion(embeds_1, embeds_2, neg_labels)
        ...
        #     print('max(l_neg_1)', torch.max(l_neg_1))
            ...
            # print('loss', loss)
            # running_loss =+ loss
        # self.log(f'pretrain/l_pos', l_pos.item(), on_epoch=True)
        # self.log(f'pretrain/l_neg', l_neg.item(), on_epoch=True)
        ...
        # logits_12 = torch.matmul(embeds_1, embeds_1_positive.T) / self.temp

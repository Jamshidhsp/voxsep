...
class Vox2Vec(pl.LightningModule):
    def __init__(
        ...
        self.temp = temp
        self.lr = lr
        self.queue = Queue(max_size=2000, embedding_size=512)
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss = criterion(embeds_1, self.queue.get().detach().to(embeds_1.device), label.to(embeds_1.device))

        l_pos =  torch.einsum('pe, pe->p', [embeds_1, embeds_1]).unsqueeze(-1)
        l_neg =  torch.einsum('pe,ke->pk', [embeds_1, self.queue.get().detach().to(embeds_1.device)])
        N = l_pos.size(0)

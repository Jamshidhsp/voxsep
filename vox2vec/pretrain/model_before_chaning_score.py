...
#         super(SinCosEncoding, self).__init__()
#         self.dim = dim

#     def forward(self, num_swaps: int, total_swap_size: float):
#         # Encode the number of swaps and the total swap size using sin and cos functions
#         swap_enc = torch.tensor([torch.sin(num_swaps), torch.cos(num_swaps)]).to(total_swap_size.device)
#         size_enc = torch.tensor([torch.sin(total_swap_size), torch.cos(total_swap_size)]).to(total_swap_size.device)

#         # Concatenate the encodings and return them
#         encodings = torch.cat([swap_enc, size_enc], dim=-1)
#         return encodings
    
# class CustomSinCosScoring(nn.Module):
#     def __init__(self):
#         super(CustomSinCosScoring, self).__init__()
#         self.sin_cos_encoding = SinCosEncoding(dim=128)  # You can adjust the dimension as needed

#     def forward(self, num_swaps: int, total_swap_size: float):
#         # Generate the sin/cos encoding based on the number of swaps and total swap size
#         scores = self.sin_cos_encoding(num_swaps, total_swap_size)
        
#         # Optionally, scale the scores (you can tune this based on your needs)
#         scores = scores * 0.1  # Scaling factor for the scores
...
# class CustomSwapScoring(nn.Module):
#     def __init__(self, base_score=1.0, swap_penalty=0.1):
#         super(CustomSwapScoring, self).__init__()
#         self.base_score = base_score  # Base score for no swap
#         self.swap_penalty = swap_penalty 
#         # self.gamma = torch.nn.Parameter(torch.tensor(0.5))# Penalty for larger swaps and more swaps
#         self.gamma = torch.tensor(0.5)
#     def forward(self, num_swaps, total_swap_size):
#         # Base score + penalty based on swap count and swap size
#         # score = self.base_score + num_swaps * self.swap_penalty + total_swap_size * self.swap_penalty
#         # print(self.gamma, num_swaps, total_swap_size)
#         score = 1 - torch.exp(-self.gamma * num_swaps*total_swap_size)
#         # print(score)
#         return score
...
class Vox2Vec(pl.LightningModule):
    def __init__(
        ...
        # self.swap_scoring = CustomSwapScoring()
        # self.custom_sin_cos_scoring = CustomSinCosScoring
    
    def extract_features_across_scales(self, feature_pyramid):
        pooled_features = []
        # feature_pyramid = feature_pyramid[-2]
    ...
    def _vox_to_vec(self, patches: torch.Tensor) -> torch.Tensor:
        # feature_pyramid = [self.backbone(patches)[-1]]
    ...
    #                     size = np.random.randint(16, max_swap_size)
    ...
    def generate_negatives(self, patches, num_negatives=4):
        ...
        # Loop through each patch in the batch
        for patch_idx in range(patches.size(0)):  # patches.size(0) is the batch size
            # negative_patch = patches[patch_idx].clone()  # Clone the current patch for each patch in the batch
            # total_swap_size = 0  # Initialize total swap size

            for i in range(num_negatives):
                negative_patch = patches[patch_idx].clone() 
                total_swap_size = 0
                num_swaps = np.random.randint(10, max_num_swaps)  # Number of swaps per patch
                max_swap_size = int(0.7* patches.size(-1))  # Limit swap size to half the patch dimension

                for _ in range(num_swaps):
                    size = np.random.randint(20, max_swap_size)
                    # size=20
                    total_swap_size += size#**3  # Accumulate total swapped volume
                    src_h, src_w, src_d = np.random.randint(0, patches.size(-1) - size, 3)
                    des_h, des_w, des_d = np.random.randint(0, patches.size(-1) - size, 3)
                    # Swap regions within the negative patch
                    negative_patch[:, src_h:src_h+size, src_w:src_w+size, src_d:src_d+size] = \
                ...
                # Compute score based on number of swaps and total swap size
                relative_num_swaps = num_swaps / max_num_swaps
                # print('relative_num_swaps', num_swaps)
                relative_swap_size = total_swap_size / (max_swap_size*max_num_swaps)
                # score = self.swap_scoring(num_swaps/10, total_swap_size/(max_swap_size*10))
                score = torch.sin(torch.tensor((np.pi/2) * relative_num_swaps)) * torch.sin(torch.tensor((np.pi/2) * relative_swap_size))
                # score = self.custom_sin_cos_scoring(num_swaps, total_swap_size)
                scores.append(score)

                negatives.append(negative_patch)  # Append the generated negative patch
        negatives = torch.stack(negatives)  # Stack the negatives into a single tensor
        scores = torch.tensor(scores).to(patches.device)  # Ensure scores are in the correct device
    ...
    def training_step(self, batch, batch_idx):
        ...
        # Ensure backbone and proj_head are in training mode
        self.backbone.train()
        ...
        scores = scores.reshape(-1, num_negatives)
        ...
        # slices = torch.cat([negative_patches[i, 0, :, :, 32] for i in range(4)], dim=1)
        # plt.imshow(slices.detach().cpu().numpy())
        # plt.savefig(str(batch_idx)+'.png')

        global_negatives = global_negatives.reshape(-1, num_negatives, proj_dim)

        logits = torch.einsum('nc,nkc->nk', global_anchor, global_negatives)/0.07
        softmax_logits = F.softmax(logits, dim=1)
        # scores = self.scoring_mlp(global_negatives.view(-1, self.proj_dim)).view(-1, num_negatives)
        # scores = scores + 0.01*torch.randn(scores.shape).to(scores.device)
        # scores = torch.log(scores/0.1)
        # softmax_scores = F.softmax(scores, dim=-1)
        # Compute the loss
        # global_loss = F.mse_loss(softmax_logits, scores)
        print('scores', scores)
        print('logits', logits)
        # global_loss = F.binary_cross_entropy_with_logits((logits), scores) #+ F.kl_div(softmax_logits, F.softmax(scores))
        global_loss = F.cross_entropy(logits, scores.argmax(dim=1))

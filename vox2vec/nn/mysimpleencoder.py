...
class MySimpleEncoder(nn.Module):
    def __init__(
            self, 
            in_channels: int, 
            base_channels: int, 
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        feature_pyramid.insert(len(feature_pyramid), x)
        return [feature_pyramid[0], feature_pyramid[-1]]


class SimpleLinearHead(nn.Module):
    def __init__(self, base_channels: int, num_scales: 1, num_classes: int) -> None:
        super().__init__()
        ...
        # self.layers = nn.ModuleList([
        #     nn.Conv3d(512, num_classes, kernel_size=1, bias=(i == 0))
        #     for i in range(num_scales)
        # ])


        self.up = nn.Upsample(scale_factor=2**5, mode='nearest')
        self.num_scales = num_scales
    ...
    def forward(self, feature_pyramid: Sequence[torch.Tensor]) -> torch.Tensor:
        ...
        feature_pyramid = [self.layers[0](feature_pyramid[-1])]
        x = feature_pyramid[-1]
        # for fmap in reversed(feature_pyramid[:-1]):
        x = self.up(x)
        x = x[(..., *map(slice, x.shape[-3:]))]
        # x += fmap
        return x
        # return x.unsqueeze(0)
    # remember me for the size matching


class SimpleNonLinearHead(nn.Module):
    def __init__(self, base_channels: int, num_scales: int, num_classes: int) -> None:
        ...
        self.single_up_block = nn.Upsample(scale_factor=8*3, mode="nearest")
        self.new_final_block = nn.Conv3d(512, num_classes, kernel_size=1)
    ...
    def forward(self, feature_pyramid: Sequence[torch.Tensor]) -> torch.Tensor:
        ...
        x = feature_pyramid[-1]
        x = self.bottom_block(x)
        x = self.single_up_block(x)
        x = self.new_final_block(x)

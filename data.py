...
class PretrainDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        patches_1, patches_2, voxels_1 = zip(*views)
        ...
        return patches_1, patches_2, voxels_1
...
def sample_views(
    ...
    patch_2 = augmentations(patch_1, window_hu, min_window_hu, max_window_hu)

    
    # voxel_list_positive, voxel_list_negative = neighboring_sampling(roi_voxels_1)
    ...
    return patch_1, patch_2, roi_voxels_1[indices], roi_voxels_1[indices]

    # return patch_1, voxel_list_positive, voxel_list_negative
...
num_positives, num_negative = 5, 5

def neighboring_sampling(voxels_list, sampling = 2):
    ...
    while counter <sampling:
        ...
        valid_1 = np.all((voxels_numpy >= voxels[random_sample_indice]-step) & (voxels_numpy < voxels[random_sample_indice]+step), axis=1)
        ...
        positive_voxels = np.asarray(positive_voxels)
        ...
        negative_voxels = np.asarray(negative_voxels)
        
        if len(negative_voxels) == num_negative:
            if len(positive_voxels) == num_positives:

                positive_list.append(np.asarray(positive_voxels))
                negative_list.append(np.asarray(negative_voxels))
                counter+=1
    positive_list_all.append(np.asarray(positive_list))
    negative_list_all.append(np.asarray(negative_list))
    return np.asarray(positive_list_all), np.asarray(negative_list_all)
...
def augmentations(image, window_hu, min_window_hu, max_window_hu):
# def sample_view(image, voxels, anchor_voxel, patch_size, window_hu, min_window_hu, max_window_hu, rotate_angle):
    # return image, voxels
